...
// Any returns true if there is any error.
...
// Request parses a fasthttp.Request into data and validates it based
// on the given schema.
func Request(ctx *fasthttp.RequestCtx, data any, schema Schema) (Errors, bool) {
	errors := Errors{}
	if err := parseRequest(ctx, data); err != nil {
...
func parseRequest(ctx *fasthttp.RequestCtx, v any) error {
	...
	if contentType == "application/x-www-form-urlencoded" {
		...
		for i := 0; i < val.NumField(); i++ {
			...
			formValue := string(ctx.FormValue(formTag))
